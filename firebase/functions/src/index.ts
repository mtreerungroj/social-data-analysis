...
import {
  BQ_PROJECT_ID,
  BQ_DATASET_ID,
  BG_TABLE_HASHTAG_RELATIONSHIP,
  BG_TABLE_HASHTAG_COUNT,
  BG_TABLE_HASHTAG_POST_ENGAGEMENT,
  BG_TABLE_HASHTAG_ENGAGEMENT_BY_TIME,
} from "./env.json";
...
export const getHashtagEngagementByTime = onRequestCORS(async (request, response) => {
  const hashtag: string = request.query.hashtag as string;
  const query = `SELECT * FROM ${'`'}${BQ_PROJECT_ID}.${BQ_DATASET_ID}.${BG_TABLE_HASHTAG_ENGAGEMENT_BY_TIME}${'`'} WHERE hashtag = "${"#" + hashtag}"`;
  const res = await _query(query);
